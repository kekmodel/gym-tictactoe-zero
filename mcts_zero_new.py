...
# state를 주체별 각각 4수까지 저장해서 new_state로 만듦 -> (9, 3, 3) array
...
class MCTS(object):
    ...
    def _convert_state(self, state):
        '''state의 주체별 최대 4수까지 history를 저장해서 새로운 state로 구성
        '''
    ...
    def init_edge(self, pr=0):
        ...
        if pr == 0:
            ...
            # root node or expand node(방문횟수 일정 수 이상)이면
...
PlayMarkO: %d   WinMarkO: %d' %
    ...
    # 데이터 저장
    print("data saved")
    np.save('data/state_memory_new_5.npy', zero_play.state_memory)
    np.save('data/edge_memory_new_5.npy', zero_play.edge_memory)
