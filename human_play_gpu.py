...
class HumanVsAi:
    def __init__(self):
        ...
        self.ai = MCTS('data/model_s800_g800_e64xl.pickle')
        self.current_user = None
    ...
    def select_action(self, state):
        if self.current_user == PLAYER:
            ...
            self.ai.current_user = self.current_user
            action = self.ai.simulation(state)
        return action
...
if __name__ == '__main__':
    ...
    for game in range(GAMES):
        ...
        while not done:
            ...
            manager.ai.player_color = player_color
            if step_play < 2:
                manager.ai.tau = 1
            else:
                manager.ai.tau = 0
        ...
        if done:
            ...
            print('- FINAL -')
            print(env.board[PLAYER] + env.board[OPPONENT] * 2, '\n')
            manager.ai = MCTS('data/model_s800_g800_e64xl.pickle')
            time.sleep(2)
